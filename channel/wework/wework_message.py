...
class WeworkMessage(ChatMessage):
    def __init__(self, wework_msg, wework, is_group=False):
        try:
            ...
            self.msg_id = wework_msg.get('MsgId') or wework_msg['data'].get('msg_id') or wework_msg['data'].get('conversation_id', wework_msg['data'].get('room_conversation_id'))
            ...
            if wework_msg["type"] == 11041:  # 文本消息类型
                ...
                # 打印完整原始消息，用于排查 msgId 和引用消息字段结构
                logger.info(f"[WX][原始消息完整] {json.dumps(wework_msg, ensure_ascii=False)}")
